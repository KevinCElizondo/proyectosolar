# Manual de Identidad Visual de SolarFluidity

## Paleta de Colores

### Colores Principales
- **Solar Blue**: #1E88E5 - Color principal que representa confianza y tecnología
- **Energy Yellow**: #FDD835 - Color secundario que representa la energía solar
- **Eco Green**: #43A047 - Color que representa sostenibilidad

### Colores Secundarios
- **Cloud White**: #FFFFFF - Fondo principal
- **Night Blue**: #1A237E - Para textos importantes
- **Gray Scale**: #F5F5F5, #E0E0E0, #9E9E9E - Para elementos de interfaz

## Tipografía

### Principal
- **Familia**: Inter
- **Usos**: Títulos, encabezados y texto destacado

### Secundaria
- **Familia**: Source Sans Pro
- **Usos**: Cuerpo de texto, párrafos y elementos de interfaz

## Logotipo

### Versiones
- **Principal**: Logo a color sobre fondo blanco
- **Monocromático**: Versión en blanco y negro
- **Invertido**: Logo en blanco sobre fondos oscuros

### Espaciado y Tamaños
- Espacio mínimo alrededor del logo: 20px
- Tamaño mínimo: 32px de altura

## Elementos de Interfaz

### Botones
- **Primario**: Fondo Solar Blue, texto blanco
- **Secundario**: Borde Solar Blue, texto Solar Blue
- **Terciario**: Texto Solar Blue sin borde

### Formularios
- Inputs con borde ligero (#E0E0E0)
- Focus state en Solar Blue
- Mensajes de error en rojo (#D32F2F)

### Iconografía
- Set de iconos: Material Design Icons
- Color principal: Night Blue
- Tamaños: 16px, 24px, 32px

## Aplicaciones

### Web
- Navbar con fondo blanco y logo principal
- Footer con fondo Night Blue y logo invertido
- Cards con sombras suaves y bordes redondeados

### Email
- Encabezado con logo centrado
- Cuerpo con fondo blanco
- Footer con información de contacto

### Documentación
- Títulos en Solar Blue
- Enlaces en Energy Yellow
- Bloques de código con fondo gris claro
